...
def ask_followings(potential_advised_followings = None):
    ...
    if len(potential_followers) > 0:
        print("Users we believe are likely to followback: ")
        for id in potential_followers:
            user = api.get_user(id)
            print(user.screen_name)
    else:
        print("No user was predicted to followback")
    print("You can try following everyone, we simply don't think they will all followback.")
